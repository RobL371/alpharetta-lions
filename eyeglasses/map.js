...
var storeLocationDataUrl = 'data/eyeglasses.txt';

//The URL to the icon image. 
var iconImageUrl = 'images/EyeglassIcon.png';
...
function initialize() {
    ...
    map = new atlas.Map('myMap', {
        center: [-84.29451948348593, 34.07560493919262],
        zoom: 11,
        view: 'Auto',
        showLogo: false,
        showFeedbackLink: false,
                ...
                authOptions: {
                    ...
                    subscriptionKey: 'PpRnWu_ZvXWSiG9a6sJMQgJyYaZihif05gv1m_Iu8Q4'
                }
    });
...
async function setMapToUserLocation() {
    ...
    navigator.geolocation.getCurrentPosition(function (position) {
        ...
        alert('Cannot use your location. Setting map center to downtown Alpharetta');
        //switch (error.code) {
    ...
    if (camera.zoom < maxClusterZoomLevel) {
        ...
        data.forEach(function (shape) {
            if (shape instanceof atlas.Shape) {
                ...
                distances[shape.getId()] = Math.round(atlas.math.getDistanceTo(centerMarker.getOptions().position, shape.getCoordinates(), 'miles') * 100) / 100;
            }
        });
        ...
        data.forEach(function (shape) {
            ...
            html.push('<div class="listItem" onclick="itemSelected(\'', shape.getId(), '\')"><div class="listItem-title">',
                ...
                //Get a formatted address line 2 value that consists of City, AdminDivision, and PostCode.
...
function showPopup(shape) {
    ...
    var distance = Math.round(atlas.math.getDistanceTo(centerMarker.getOptions().position, shape.getCoordinates(), 'miles') * 100) / 100;

    var html = ['<div class="storePopup">'];

    html.push(
        '<a href="',
        getGMapsLink(properties),
        '" target="_blank">',
        ...
        '</br>',
        getAddressLine2(properties),
        '</div></div></a><div class="popupContent">',
...
//Creates an addressLine2 string consisting of City, AdminDivision, and PostCode.
...
//Creates an addressLine2 string consisting of City, AdminDivision, and PostCode.
function getGMapsLink(properties) {
    var html = ['https://www.google.com/maps/search/?api=1&query='];

    var html2 = [properties['Company']];

    html2.push(' ', properties['AddressLine'], ' ', properties['City']);

    if (properties['AdminDivision']) {
        html2.push(', ', properties['AdminDivision']);
    }

    if (properties['PostCode']) {
        html2.push(' ', properties['PostCode']);
    }
    html.push(encodeURIComponent(html2.join('')));
